...
object Day04 extends AdventApp(4) {
  ...
  def parse(line: String): (LocalDateTime, String) = {
  ...
  type Desired = Map[(Guard,Day), List[Sleep]]

  override def go(): Unit = {
    val pathological = "1567-11-05 23:58".toLocalDateTime
    println(pathological.toNearestLocalDate)
  }
