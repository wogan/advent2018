...
object Day09 extends AdventApp(9) {
  ...
  val testInput: Vector[String] =
    """10 players; last marble is worth 1618 points: high score is 8317
      ...
      |30 players; last marble is worth 5807 points: high score is 37305""".testInput

  val regex: Regex = "(?<players>\\d+) players; last marble is worth (?<points>\\d+) points(: high score is (?<score>\\d+))?".r

  def parse(line: String): (Int, Int, Option[Int]) = {
    implicit val m: Regex.Match = regex.findFirstMatchIn(line).get
    ('players, 'points, 'score)
  }

  override def go(): Unit = {
    testInput map parse foreach println
    println(parse(input.head))
    println(s"Part One: ${"shrug"}")
  }

  case class Circle(size: Int = 1, current: Int = 0) {
    // when size is one the circe contains only a single marble '0'

    def addMarble: (Circle, Option[Int]) = {
      val s
    }
  }

  def score(players: Int, lastMarble: Int): Int = {
    0
  }
